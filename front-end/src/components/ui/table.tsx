...
>(({ className, ...props }, ref) => (
    <table
        ref={ref}
        className={cn(
            'relative w-full caption-bottom sm:text-sm text-xs',
            className,
        )}
        {...props}
    />
...
>(({ header = false, subrow = false, className, ...props }, ref) => (
    <tr
        ref={ref}
        className={cn(
            'bg-primary-50 border-b border-primary-100 transition-colors hover:bg-primary-100 data-[state=selected]:bg-muted',
