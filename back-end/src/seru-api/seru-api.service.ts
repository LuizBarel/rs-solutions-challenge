...
export class SeruApiService {
    ...
    private readonly apiURL: string = "https://integration.plataformaseru.com.br/v1";
     ...
     */
    async searchInAPI(type: any, params: Object) {
        let response: AxiosResponse

        try {
            response = await firstValueFrom(this.httpService.get(`${this.apiURL}/${type}`, {
                params: params,
                headers: {
    ...
    async generateToken(): Promise<any> {
        ...
        try {
            const response: AxiosResponse = await firstValueFrom(this.httpService.post(`${this.apiURL}/oauth/token`, {
    ...
    async getAllOrders(): Promise<any> {
        ...
        const { currentDateFormatted, minusOneHourDateFormatted } = getFormattedDates();

        const params: Object = {
            initialUpdatedAt: minusOneHourDateFormatted,
            finalUpdatedAt: currentDateFormatted,
        }
        const data: Promise<Array<any>> = this.searchInAPI("orders", params);
     ...
     * @returns Promise<any> data
     */
    async getAllCashiers(): Promise<any> {
        if (this.token == null) {
            await this.scheduleGenerateToken();
        }
        ...
        const data: Promise<Array<any>> = this.searchInAPI("cashiers", params)
     ...
     * @returns Promise<any> data
     */
    async getAllTaxInvoices(): Promise<any> {
        if (this.token == null) {
            await this.scheduleGenerateToken();
        }
        ...
        const data: Promise<Array<any>> = this.searchInAPI("tax-invoices", params)
     ...
     */
    sendDebugMessage(condition: boolean, successMessage: string, failureMessage: string): void {
    ...
    async scheduleGenerateToken(): Promise<any> {
        ...
        this.sendDebugMessage(typeof this.token == 'string', "Token gerado com sucesso", "O token não foi gerado")
    }
    ...
    async scheduleGetOrders(): Promise<any> {
        let data: Array<any> = await this.getAllOrders()

        this.sendDebugMessage(data.length > 1, "Pedidos recuperados com sucesso", "Não há pedidos para recuperar")
    }
    ...
    async scheduleGetCashiers(): Promise<any> {
        let data: Array<any> = await this.getAllCashiers()

        this.sendDebugMessage(data.length > 1, "Caixas recuperados com sucesso", "Não há caixas para recuperar")
    }
    ...
    async scheduleGetTaxInvoices(): Promise<any> {
        let data: Array<any> = await this.getAllTaxInvoices()

        this.sendDebugMessage(data.length > 1, "Notas recuperados com sucesso", "Não há notas para recuperar")
    }
}
